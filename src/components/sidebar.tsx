...
type Status = 'closed' | 'sidebarOpen' | 'modalOpen'

export default function Sidebar() {
  const { data } = api.boards.getAllNames.useQuery()
  const [status, setStatus] = useState<Status>('closed')
  const isSidebarOpen = status !== 'closed'
  ...
  return (
    <>
      ...
      >
        <div className="hidden h-full flex-col gap-6 md:flex md:w-[16.25rem] lg:w-[18.75rem]">
          ...
          <div className="scrollbar-hidden flex grow flex-col gap-4 overflow-y-auto">
            <BoardsNav
              ...
              onCreateBoardClick={() => setStatus('modalOpen')}
            />
      ...
      <NewBoardModal
        ...
        onOpenChange={(isOpen: boolean) =>
          setStatus(isOpen ? 'modalOpen' : 'sidebarOpen')
        }
      />
